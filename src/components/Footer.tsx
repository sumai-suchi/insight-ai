                        ...
                        >
                            {features.map((feature) => (
                                <motion.li key={feature.name} variants={fadeInUp}>
                                    <Link
                                        href={feature.href}
                                        className="text-gray-900 hover:text-[#3B82F6] text-sm transition-colors duration-200 flex items-center group"
                                    >
                                        <feature.icon className="h-3.5 w-3.5 mr-2 text-gray-500 group-hover:text-[#3B82F6]" />
                                        {feature.name}
                                        {feature.badge && (
                                            <span className="ml-2 px-1.5 py-0.5 bg-[#FBBF24] text-gray-900 text-[10px] font-bold rounded">
                                                {feature.badge}
                                            </span>
                                        )}
                                    </Link>
                            ...
                            <form className="space-y-3">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="block w-full pl-10 pr-3 py-2.5 bg-gray-100 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:border-transparent text-sm"
                                        required
                                    />
                                </div>
                                <motion.button
                        ...
                        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2">
                            {['privacy', 'terms', 'cookies', 'sitemap', 'accessibility'].map((item) => (
                                <Link 
                                    key={item}
                                    href={`/${item}`} 
                                    className="text-sm text-gray-600 hover:text-[#3B82F6] transition-colors capitalize"
                                >
                                    {item}
                                </Link>
                            ))}
