...
const EditorApplyForm = () => {
  ...
  useEffect(() => {
    fetch("/api/checkout?userId=")
      .then((res) => res.json())
      .then((data) => console.log("data is myh", data));
  }, []);
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#BDE8F5]/30 p-4">
      ...
      >
        ...
        <div className="bg-[#0F2854] p-8 text-center relative">
          ...
          <p className="text-[#BDE8F5] text-sm opacity-80">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Read-only Name */}
            <div>
              <label className="block text-sm font-semibold text-[#1C4D8D] mb-2">
                Full Name
              </label>
              <input
                type="text"
          ...
          <div className="bg-[#BDE8F5]/20 p-4 rounded-xl flex justify-between items-center border border-[#BDE8F5]">
            <div className="text-center flex-1">
              <p className="text-xs text-[#1C4D8D] uppercase font-bold">
                Current Plan
              </p>
              <p className="text-lg font-semibold text-[#0F2854] capitalize">
                {userData.plan}
              </p>
            </div>
